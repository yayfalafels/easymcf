...
- Run test suites against the `~/env` operational venv (it's part of the app's lifecycle); one-off diagnostic probes for a surfaced test failure belong in `~/.dev/dev-env` instead — never a throwaway one-off env for either. See [local-infra-navigation](../skills/local-infra-navigation/SKILL.md#python-virtual-environments--hard-rule).
