...
Implements [010-01-requirements.md](../../../docs/releases/010/010-01-requirements.md)'s "Automated apply" section (REQ-APPLY-01..09). Site-specific selectors live in [mycareerfutures](../mycareerfutures/SKILL.md); wait/retry mechanics live in [selenium](../selenium/SKILL.md).
...
2. Poll for the apply button, bounded retries with delay (REQ-APPLY-07 — see [selenium](../selenium/SKILL.md) for the wait pattern). Button found and clickable → click → proceed. Not found, but an "already applied" status message is present → treat as **success**, not a separate failure code. Not found, but a "closed"/"no longer available" message is present → `post_closed`. Retries exhausted with neither signal → `unable_to_apply`.
