...
Easy MCF streamlines the job search and application process for job seekers in Singapore using [MyCareersFuture](https://www.mycareersfuture.gov.sg/). Current release: `010` (POC local) — a locally-run Python (Flask) backend + SQLite database + AngularJS frontend, re-platforming the `jobsearch` prototype's search/CRM/apply-automation workflow without repeating `mcfpipe`'s premature cloud-infrastructure investment. The app has user accounts (email/password and Google sign-in) with per-user data, and runs the MCF automation in `fixture` mode (recorded pages) or `live` mode (the real site, human-triggered only). Source of truth for current scope: [docs/releases/release-roadmap.md](docs/releases/release-roadmap.md) and [docs/releases/010/design/010-01-requirements.md](docs/releases/010/design/010-01-requirements.md) — read the relevant requirements before implementing anything, don't infer scope from code alone.

## Repo layout

- `easymcf/` (this repo) — the `010` build:
  - `easymcf/` — the Flask app package (API, auth, services, db, Playwright automation).
  - `frontend/` — the AngularJS app, vendored and served by Flask from the same origin (no npm/build step).
  - `scripts/` — tracked operational toolkit (`resetdb.py`, `db_util.py`, `envcheck.py`, `restart.sh`, validation utilities).
  - `seed/`, `tests/` — seed SQL and the pytest tiers with their fixtures.
- `docs/releases/` — roadmap, per-release milestones, design docs (`<release>/design/`), and feature trackers (`<release>/features/`).
...
[docs/releases/010/design/010-prototype.md](docs/releases/010/design/010-prototype.md) already extracts the implementation-level detail from both sibling repos that most work needs — check there before reading `jobsearch/`/`mcfpipe/` source directly.

## Claude Code tooling for this project

Scoped in [docs/releases/010/design/010-claude-agent.md](docs/releases/010/design/010-claude-agent.md).
...
- Process and docs: `feature-implementation-guide` (feature-tracker pattern, implementation and validation conventions), `design-audit` (release design-doc review), `documentation-style` (prose rules), `markdown-tables` (table rules).

**Subagents** (`.claude/agents/`) — delegate to these for role-scoped work: `product-manager` (requirements/milestones/scope/QA), `architect` (data model/architecture/design), `backend-api-developer` (Flask/SQLite), `frontend-ui-developer` (AngularJS), `automation-engineer` (scraping/apply browser automation), `testing-validation` (test strategy, test runs, requirement validation), `documentation-editor` (doc style audits and repairs).
...
- Cloud deployment and CI/CD are out of scope for `010` — don't introduce AWS/cloud SDKs, credentials, or infrastructure as part of this release's work. The app stays a local process bound to `127.0.0.1`. The one external cloud dependency is the Google OAuth client behind Google sign-in: its client id lives in `.env` and its secret under `.secrets/`, never in the repo. The one CI/CD exception is the GitHub Actions workflow that builds the MkDocs site and publishes it to GitHub Pages ([010.18](docs/releases/010/features/010.18-release-documentation.md)). It builds and publishes documentation only, runs no app build, test, or release step, and uses no cloud credentials.
