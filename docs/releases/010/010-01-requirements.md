...
This document defines *what* this release must do. It is the input to the milestones that follow it: test strategy, test cases, data model, architecture, design, local dev/test environment, seed data, and the three functional build; job leads tracking, search by keywords, apply automation. Those milestones own *how* — schema, API shape, algorithms, folder structure, test tooling. This document intentionally stops short of that: where a decision is genuinely deferred
...
- [010-prototype.md](010-prototype.md) — implementation-level extraction from `jobsearch` and `mcfpipe`, sufficient for most of this document and the design stage. Use this first.
- `jobsearch/` — the working prototype this release formalizes and re-platforms onto a local Python API + SQLite + AngularJS stack. Go to source directly (`agent.py`, `database.py`, `mcf_profile.py`, `match.py`/`text.py`, `apply.py`, `gsheet/applicationtrackingapp.gs`) only for detail beyond what [010-prototype.md](010-prototype.md) captures.
- `mcfpipe/`  — an earlier, more ambitious rebuild of the same product; its data model and API design are worth reusing, its cloud infrastructure pivot (serverless, NoSQL, multi-tier VPC, IaC, full CI/CD) undertaken before the functional product worked reliably is the specific mistake release `010` is scoped to avoid. Go to source directly (`docs/data_model.md`, `docs/database_api.md`, `docs/architecture.md`, `docs/enhancements.md`) only for detail beyond what [010-prototype.md](010-prototype.md) captures.
...
- **lead:** A single user's personal, trackable instance of a post for one track, followed from initial interest through to a closed outcome (success or fail). Created when the user promotes a post under a track; its first stage is `PROSPECT`.
...
- **lead activity log:** The append-only, timestamped record of updates to a lead (stage transitions, contact logged, notes edits, deadline changes); its latest entry drives auto-expiry and backs the lead's activity history (REQ-CRM-08).
...
On demand from the UI, easymcf runs a search against `MCF` for a track, retrieves matching posts, and fetches full detail for any post not already on file, so the results a user reviews are never a stale or partial snapshot. The user configures one or more tracks — a role at a given seniority level — each carrying its own search keywords, target salary, and maximum post age. 
Every post is scored for relevance against the track(s) it matches so the user can prioritize what to review first, and a post found elsewhere can also be entered manually and folds into the same review and scoring flow rather than living outside the system.

This reuses the job-search workflow and matching approach already proven in the `jobsearch` prototype largely as-is for 010 — see [010-prototype.md](010-prototype.md) for its scraping, deduplication, and scoring mechanics. One improvement is made for this release: results must be saved as they are found rather than only once at the end of a full run.

The `jobsearch` prototype's match score combined a primitive title-keyword component with a salary-percentile component; the latter carried a brittle dependency on industry classification (see [010-prototype.md](010-prototype.md)) and is deprecated — `010` carries forward only the title-keyword component. Moving beyond keyword/title matching to NLP-based semantic matching of title and job description is out of scope for `010` and targeted for release `020` (MVP cloud).

- **REQ-SRCH-01** User can define one or more tracks (role + seniority), each carrying exactly one search profile (keyword(s), minimum salary, maximum post age, minimum match score; employment type defaults to Full Time), configurable by the user via the UI.
...
- **REQ-SRCH-08** Each post is associated with the search run (and implicitly the track) that discovered it, or flagged as manually entered. A post may match more than one track, with each match scored independently (REQ-SRCH-09); the one track a resulting lead is filed under is chosen by the user at promotion (REQ-CRM-01), not assigned automatically.
- **REQ-SRCH-09** Each post has a computed match score (0–1) per track it matches, visible to the user for prioritization, using the `jobsearch` prototype's primitive title-keyword approach (see [010-prototype.md](010-prototype.md)) — its salary-percentile component is deprecated and not carried forward into `010` (see Out of scope). posts are screened by a configurable maximum age and minimum score. The score is recorded together with its scoring method so semantic/NLP-based matching (title + job description, targeted for release `020`) can be added later without a schema change.
...
- **REQ-CRM-01** User can promote a post (scraped or manual) into a tracked lead under one track, owned by that user and that track, defaulting to status `OPEN` and stage `PROSPECT`. If the post matches more than one track (REQ-SRCH-08), the user selects which track to promote it under. A post can be promoted into at most one lead; once promoted, it is no longer available for promotion under a different track.
...
The apply mechanics and full outcome vocabulary already implemented in the `jobsearch` prototype are proven and carry forward as the starting point for design — see [010-prototype.md](010-prototype.md) — including its explicit, deliberate non-goal: posts that require a multi-step questionnaire are detected and left for the user, not automated.
...
- **REQ-APPLY-05** A post whose submission requires a multi-step questionnaire is detected (submit fails on the expected 1-click flow) and recorded as `questionnaire_required` without the run attempting to complete it or aborting; the user completes it manually. Automating questionnaire flows is out of scope for `010`.
...
- **REQ-APPLY-10** An apply attempt recording status `post_closed` or `post_unavailable` automatically closes the lead with close reason `apply_failed` (REQ-CRM-02) — the underlying post can no longer be pursued, so no retry is offered. Any other failure status (`cv_selector_error`, `unable_to_apply`, `cv_not_found`, `invalid_input`) leaves the lead at stage `TOAPPLY` for the user to resolve and retry.
- **REQ-APPLY-11** User can remove a queued (not yet run) application from the queue. Removal closes the owning lead with close reason `withdrawn` (REQ-CRM-02) rather than reverting it to stage `PROSPECT` — a removed application is not recycled into a future batch.

### Backend data platform

The backend is a locally-run Python service exposing the search, pipeline, and apply capabilities above to the frontend, backed by a local SQLite database — replacing the prototype's scripts-plus-spreadsheet coupling with a proper API and a single source of truth for application data. See [010-prototype.md](010-prototype.md) for a reference API design this can build on.

- **REQ-PLAT-01** Backend is implemented in Python as a locally-run REST/HTTP+JSON service (e.g. Flask), exposing a generic, schema-validated CRUD interface per entity (`GET/PUT/DELETE /{table}/{id}`, `POST /{table}`, `POST /{table}/batch`, `GET /{table}/search`, `POST /{table}/delete`) rather than bespoke endpoints per feature, per `mcfpipe`'s Database API design. 
- **REQ-PLAT-02** All domain data (posts, leads, applications, tracks/search profiles, match scores, run history) is persisted in a local SQLite database, replacing Google Sheets as both config store and data store.
...
The frontend is a single local web application the user interacts with directly, replacing every spreadsheet tab and menu action the prototype relied on with an equivalent screen or control — see [010-prototype.md](010-prototype.md) for what those were. 010 commits to AngularJS per the roadmap's feature table.
