...
**ARCH-BOT-02** Browser automation is split by responsibility. Search and apply use `MCFBrowser`. Session establishment uses `SingpassBrowser` from `easymcf/automation/singpass_browser.py`. Both expose fixture and live implementations selected by `MCF_MODE`, and services obtain them through factories rather than instantiating concrete browsers. Fixture implementations route requests to checked-in HTML before dispatch. The connection browser automates navigation, QR extraction, callback detection, account extraction, and state export. The user's Singpass approval remains manual per REQ-APPLY-06.
