...
- **architecture doc** — [010-architecture.md](010-architecture.md) — `ARCH-RUN-01..10` fixes the runtime/process model and repo layout, `ARCH-STO-01..07` fixes storage, seed, and reset, `ARCH-BOT-01..05` fixes Playwright, `ARCH-TEST-01..09` fixes tier mechanics. This document does not restate any of these. It operationalizes them.
- **test-strategy doc** — [010-test-strategy.md](010-test-strategy.md) — `STRAT-SILO-01..07` names the helper scripts (`db_util.py`, `api_tester.py`) and the frontend silo this document gives install/run instructions for. `STRAT-LOOP`, the agent's silo-aware build→test→debug table, is what section 5 below adds a preflight/install layer beneath.
- **requirements doc** — [010-01-requirements.md](010-01-requirements.md) — REQ-DEV-01..05, the requirement this whole document satisfies.
- **api doc** — [010-api.md](010-api.md) — `API-EP-07 GET /api/v1/health`, the readiness probe section 4 polls.
- **frontend-app doc** — [010-frontend-app.md](010-frontend-app.md) — the sibling design doc, elaborated under the same milestone-06 grouping this document belongs to.
...
- **infra-navigation skill** — [local-infra-navigation](../../../.claude/skills/local-infra-navigation/SKILL.md) — scaffold skill this document is written to fill in. Once milestone 07 is implemented, its content should be copied forward from here rather than re-derived.
- **deploy-cycle skill** — [deploy-and-validation-cycle](../../../.claude/skills/deploy-and-validation-cycle/SKILL.md) — same scaffold status as the infra-navigation skill. It is the golden-path checklist this document's automated loop, section 5, is meant to make redundant, tier by tier.
...
- **local-dev-env tracker** — [010.07-local-dev-env.md](../features/010.07-local-dev-env.md) — the feature tracker whose `07.05` task breakdown produces the artifacts this document's bootstrap sequence assumes already exist.
