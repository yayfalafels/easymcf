...
**TESTDATA-GAP-01 Pre-screening posts.** `jobsearch` writes every scraped card to a local SQLite `job` table and every detail-pass result to `profile`, per the **prototype extraction**'s Scrape mechanics / Posting detail fetch sections. Neither table was ever synced to the Google Sheet. Only `match.py`'s output (`screened.csv`) was, so every post in the export already cleared the prototype's own scoring. `010` carries forward no promotion-time score filter at all: every post a track's own search returns writes a `match_score` row fixed at `1.0` and promotes unconditionally, since the track's own keywords already are the whole of relevance screening, per REQ-SRCH-09. There is consequently no below-threshold or screened-out post state in `010` for the generator to reconstruct. The historical `track-assignment.csv`/`screened.csv` scores nonetheless carry forward into the migrated rows' `match_score`/`score_method='title_keyword_v1'` at their own real, varied values, so a later differentiating scoring method (milestone 16) inherits a realistic score distribution to evaluate against, rather than a corpus already flattened to `1.0` by this migration. `run_log`/`job_batch`, the prototype's execution-log tables, were likewise never synced. `run_log` is entirely synthesized, see `TESTDATA-MAP-07`.
...
| `post_id` | `jobid` | |
| `track_id` | `trackid` | |
| `search_match` | — | `true` for every row sourced from `track-assignment.csv`/`screened.csv` (both are search-pipeline outputs); `false` for rows belonging to a `post` tagged `src_method='manual'` (per REQ-SRCH-07, a manual post's one track is named on its request, not discovered by search) |

Each `post_track` row is paired one-to-one with a `match_score` row below, written in the same generator pass. Every post carries exactly one `post_track`/`match_score` pair, to the one track that found or was given it (`010-workflows.md`'s Decision 2) — the historical corpus's varied `match_score` values carry forward as-is (`TESTDATA-GAP-01`), but no row is filtered out by them at query time.
