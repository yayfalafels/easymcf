...
This diagram is the complete, closed list of legal `stage` transitions. `API-HOOK-01`'s `PUT /api/v1/lead/{id}` answers `409` to any transition not drawn here. Each stage advances one adjacent step forward (`APPLIED` to `CALLBACK`, `CALLBACK` to `INTERVIEW`), and `CLOSED` is reachable from every open stage.
